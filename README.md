...
| `otto/tests/cp4/test_the_model_sees_the_conversation.py` | The model is sent the conversation and not just the latest message, graded on the wire: a real HTTP endpoint records what the client posted, history arrives oldest-first ahead of the current turn, a caller with no history sends exactly the one message it always did, the budget is spent from the newest turn backwards, and a store that cannot be reached costs nobody their answer. |
...
| `otto/tests/cp5/test_the_wire_format_never_reaches_the_person.py` | Otto's own reply contract is never what a person is handed: a repaired reply that restates the wire format is refused rather than delivered, a repaired reply that answers is still delivered, a first reply about JSON is an answer and not an echo, and the refusal sentence a person reads carries no parser words while a state about the world keeps its reason. |
