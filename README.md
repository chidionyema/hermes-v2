...
| `tests/test_incident_crew561_otto_reaches_estate_state_through_the_mcp.py` | crew#561 CP3: `config.yaml` names the estate MCP under `mcp_servers.estate` with an https URL and a placeholder the runtime resolves from the pod's env dir; a literal bearer in the file fails. |
| `tests/test_incident_crew717_otto_is_seen_and_speaks.py` | crew#717 wave 1: the Langfuse observability plugin is enabled in `config.yaml` (it gates itself on credentials), its SDK is pinned in the Dockerfile, and the `edge-tts` extra ships in the image — Otto is seen (LAW 50) and can speak. |
