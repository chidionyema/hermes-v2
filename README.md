...
| `otto/tests/boot/test_cp3_explorer_lane.py` | Crew#892 CP3: the explorer lane (`OTTO_ROUTER_LANE_EXPLORE_MODEL`) is a shipped lane whose model env var is machine-listable, is not pre-exhausted at zero spend, and fans N distinct propose-fix candidates that all reach the reasoning lane's synthesis step with every call charged to the explore lane in the ledger. |
