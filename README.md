...
| `otto/boot/presence.py` | The typing indicator kept lit while a model thinks: a daemon thread re-sends `sendChatAction` every 4.5s (Telegram clears it after ~5) so a 30-second reasoning answer does not read as the bot being down. |
| `otto/boot/transport.py` | Outbound calls to the Telegram Bot API (`sendMessage`, `setWebhook`, `sendChatAction`) over stdlib `urllib.request`, and the `TelegramTransport` protocol a test fakes. |
| `otto/boot/visible_progress.py` | Crew#892 CP2: a `ProgressEditor` that posts ONE placeholder the moment the founder's instruction is read and edits that same message in place as the model works — placeholder → fresher line per step → final answer edited into the very same message. Every Bot API failure is a courtesy swallowed to obs, never a cost to the sender's reply. |
...
| `otto/tests/boot/test_transport_cp2_edits.py` | Crew#892 CP2 foundation: `send_message` hands back the message id Telegram assigned so a later edit can target it, and `edit_message` posts `editMessageText` — the two primitives an in-place progress placeholder is built on. |
| `otto/tests/boot/test_cp2_founder_sees_progress.py` | Crew#892 CP2 mechanism end-to-end through `handle_webhook_body`: the founder's answer is edited into the single placeholder it was sent into (one send, then in-place edits); a transport that cannot host edits or a placeholder Telegram refuses still delivers the answer as a normal fresh send and never loses it; unknown and blank senders get no placeholder at all. |
